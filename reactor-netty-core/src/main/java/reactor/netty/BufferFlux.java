...
import io.netty5.handler.codec.http.HttpContent; // TODO we should NOT depends on HTTP module
import io.netty5.handler.codec.http.websocketx.WebSocketFrame; // TODO we should NOT depends on HTTP module
...
public class BufferFlux extends FluxOperator<Buffer, Buffer> {
	...
	public final Flux<String> asString(Charset charset) {
		Objects.requireNonNull(charset, "charset");
		return handle((bb, sink) -> sink.next(bb.readCharSequence(bb.readableBytes(), charset).toString()));
	}
	...
	static final Function<Object, Buffer> bufferExtractor = o -> {
		...
		if (o instanceof BufferDatagramPacket envelope) {
	...
	static final Logger log = Loggers.getLogger(BufferFlux.class);
}
